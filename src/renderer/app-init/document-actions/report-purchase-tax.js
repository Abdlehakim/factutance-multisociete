(function (w) {
  ...
  AppInit.registerPurchaseTaxReportActions = function registerPurchaseTaxReportActions() {
    const SEM = (w.SEM = w.SEM || {});
    if (SEM.__purchaseTaxReportBound) return;
    SEM.__purchaseTaxReportBound = true;
    ...
    const buildPurchaseTaxReportSnapshot = () => {
    ...
    const buildPurchaseTaxReportHtml = (snapshot = {}) => {
      ...
      const reportTitle = "Rapport de taxes à l'achat";
    ...
    const updatePurchaseTaxReportActions = () => {
    ...
    const buildPurchaseTaxReportCss = () => {
    ...
    const buildReportTaxFilename = (startDate, endDate, index) => {
      ...
      return `RPTA-${start}-${end}-${safeIndex}`;
    };

    const exportPurchaseTaxReportPdf = async () => {
      ...
      const snapshot = buildPurchaseTaxReportSnapshot();
      const html = buildPurchaseTaxReportHtml(snapshot);
      const css = buildPurchaseTaxReportCss();
      let index = 1;
      let res = null;
      while (index <= 999) {
        ...
        if (res?.ok) {
          ...
          updatePurchaseTaxReportActions();
    ...
    const openPurchaseTaxReportFolder = async () => {
    ...
    const printPurchaseTaxReport = async () => {
      if (reportTaxState.loading) return;
      if (reportTaxState.error) {
    ...
    const buildPurchaseTaxReportData = async ({ startDate, endDate } = {}) => {
      ...
      let listRes = null;
      try {
        listRes = await w.electronAPI.listInvoiceFiles({ docType: "fa" });
      } catch (err) {
        return { error: String(err?.message || err || "Erreur de lecture.") };
      }
      if (!listRes?.ok || !Array.isArray(listRes.items)) {
        return {
          error: listRes?.error || "Impossible de recuperer les factures d'achat."
        };
      }
      const entries = listRes.items;
      const filtered = entries.filter((entry) => {
        const docType = String(entry?.docType || "fa")
          .trim()
          .toLowerCase();
        return docType === "fa" && isDateInRange(entry?.date, normalizedStart, normalizedEnd);
      });
    ...
    const renderPurchaseTaxReportModal = () => {
      ...
      const snapshot = buildPurchaseTaxReportSnapshot();
      reportTaxState.content.innerHTML = buildPurchaseTaxReportHtml(snapshot);
      updatePurchaseTaxReportActions();
    };

    const closePurchaseTaxReportModal = () => {
      ...
      document.removeEventListener("keydown", onPurchaseTaxReportKeyDown);
    ...
    const onPurchaseTaxReportKeyDown = (evt) => {
      if (evt.key === "Escape") {
        evt.preventDefault();
        closePurchaseTaxReportModal();
      }
    };

    const ensurePurchaseTaxReportModal = () => {
      if (reportTaxState.overlay) return reportTaxState.overlay;
      const overlay = document.createElement("div");
      overlay.id = "purchaseTaxReportModal";
      overlay.className = "swbDialog doc-history-modal report-tax-modal";
      overlay.hidden = true;
      overlay.setAttribute("aria-hidden", "true");
      overlay.innerHTML = `
        <div class="swbDialog__panel doc-history-modal__panel pdf-preview-modal__panel report-tax-modal__panel" role="dialog" aria-modal="true" aria-labelledby="purchaseTaxReportModalTitle">
          <div class="swbDialog__header">
            <div id="purchaseTaxReportModalTitle" class="swbDialog__title">Rapport de taxes à l'achat</div>
            <button id="purchaseTaxReportModalClose" type="button" class="swbDialog__close" aria-label="Fermer">
          ...
          <div class="pdf-preview-modal__body report-tax-modal__body swbDialog__msg">
            <div id="purchaseTaxReportContent" class="pdf-preview-modal__content report-tax-modal__content"></div>
          </div>
          <div class="pdf-preview-modal__actions report-tax-modal__actions">
            <div class="pdf-preview-modal__buttons report-tax-modal__buttons">
              <button id="purchaseTaxReportModalCloseFooter" type="button" class="client-search__edit">Fermer</button>
              <button id="purchaseTaxReportModalPrint" type="button" class="client-search__addSTK" disabled aria-disabled="true">Imprimer Rapport</button>
              <button id="purchaseTaxReportModalExport" type="button" class="client-search__edit" disabled aria-disabled="true">Exporter PDF</button>
              <button id="purchaseTaxReportModalOpenFolder" type="button" class="client-search__edit doc-history__open-folder" title="Ouvrir le dossier PDF" aria-label="Ouvrir le dossier PDF" disabled aria-disabled="true">
      ...
      reportTaxState.content = overlay.querySelector("#purchaseTaxReportContent");
      reportTaxState.title = overlay.querySelector("#purchaseTaxReportModalTitle");
      reportTaxState.closeBtn = overlay.querySelector("#purchaseTaxReportModalClose");
      reportTaxState.closeFooterBtn = overlay.querySelector("#purchaseTaxReportModalCloseFooter");
      reportTaxState.exportBtn = overlay.querySelector("#purchaseTaxReportModalExport");
      reportTaxState.printBtn = overlay.querySelector("#purchaseTaxReportModalPrint");
      reportTaxState.openFolderBtn = overlay.querySelector("#purchaseTaxReportModalOpenFolder");

      reportTaxState.closeBtn?.addEventListener("click", closePurchaseTaxReportModal);
      reportTaxState.closeFooterBtn?.addEventListener("click", closePurchaseTaxReportModal);
      reportTaxState.exportBtn?.addEventListener("click", exportPurchaseTaxReportPdf);
      reportTaxState.printBtn?.addEventListener("click", printPurchaseTaxReport);
      reportTaxState.openFolderBtn?.addEventListener("click", openPurchaseTaxReportFolder);
      overlay.addEventListener("click", (evt) => {
        if (evt.target === overlay) closePurchaseTaxReportModal();
      });
      return overlay;
    };

    const showPurchaseTaxReportModal = async ({ startDate, endDate } = {}) => {
      ensurePurchaseTaxReportModal();
      ...
      document.addEventListener("keydown", onPurchaseTaxReportKeyDown);
      renderPurchaseTaxReportModal();

      const data = await buildPurchaseTaxReportData({
        startDate: reportTaxState.startDate,
        endDate: reportTaxState.endDate
      });
    ...
    const promptPurchaseTaxReportRange = async () => {
      let startDate = "";
      let endDate = "";
      let hintEl = null;
      ...
      const confirmed = await w.showConfirm("Selectionnez la periode du rapport.", {
        title: "Rapport de taxes à l'achat",
        okText: "Valider",
        cancelText: "Annuler",
    ...
    const reportButton = getElSafe("btnReportPurchaseTax");
    reportButton?.addEventListener("click", async () => {
      const range = await promptPurchaseTaxReportRange();
      if (!range) return;
      await showPurchaseTaxReportModal(range);
    });
  };
})(window);
