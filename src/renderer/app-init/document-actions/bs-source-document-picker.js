(function (w) {
  ...
  const SOURCE_TYPE_DIALOG_ID = "bsSourceDocumentTypePickerDialog";
  const SOURCE_TYPE_DIALOG_TITLE_ID = "bsSourceDocumentTypePickerDialogTitle";
  const SOURCE_TYPE_DIALOG_MESSAGE_ID = "bsSourceDocumentTypePickerDialogMessage";
  const SOURCE_TYPE_DIALOG_OPTIONS_ID = "bsSourceDocumentTypePickerDialogOptions";
  const SOURCE_TYPE_DIALOG_CLOSE_ID = "bsSourceDocumentTypePickerDialogClose";
  const SOURCE_TYPE_DIALOG_CANCEL_ID = "bsSourceDocumentTypePickerDialogCancel";
  const MODAL_ID = "bsSourceDocumentPickerModal";
  const TITLE_ID = "bsSourceDocumentPickerTitle";
  const GRID_ID = "bsSourceDocumentPickerGrid";
  const STATUS_ID = "bsSourceDocumentPickerStatus";
  const REFRESH_ID = "bsSourceDocumentPickerRefresh";
  const CLOSE_ID = "bsSourceDocumentPickerClose";
  const CLOSE_FOOTER_ID = "bsSourceDocumentPickerCancel";
  const SELECT_ALL_ID = "bsSourceDocumentPickerSelectAll";
  const UNSELECT_ALL_ID = "bsSourceDocumentPickerUnselectAll";
  const CONFIRM_ID = "bsSourceDocumentPickerConfirm";
  const PAGE_ID = "bsSourceDocumentPickerPage";
  const PAGE_INPUT_ID = "bsSourceDocumentPickerPageInput";
  const TOTAL_PAGES_ID = "bsSourceDocumentPickerTotalPages";
  const PREV_ID = "bsSourceDocumentPickerPrev";
  const NEXT_ID = "bsSourceDocumentPickerNext";
  const SEARCH_INPUT_ID = "bsSourceDocumentPickerSearchNumber";
  const SUPPLIER_FIELD_ID = "bsSourceDocumentPickerSupplierField";
  const SUPPLIER_INPUT_ID = "bsSourceDocumentPickerSupplier";
  const SUPPLIER_PANEL_ID = "bsSourceDocumentPickerSupplierPanel";
  const SUPPLIER_LABEL_ID = "bsSourceDocumentPickerSupplierLabel";
  const YEAR_SELECT_ID = "bsSourceDocumentPickerYearFilter";
  const YEAR_MENU_ID = "bsSourceDocumentPickerYearMenu";
  const YEAR_LABEL_ID = "bsSourceDocumentPickerYearLabel";
  const YEAR_DISPLAY_ID = "bsSourceDocumentPickerYearDisplay";
  const YEAR_PANEL_ID = "bsSourceDocumentPickerYearPanel";
  const PAGE_SIZE = 20;
  ...
  const normalizeDocType = (value, fallback = "facture") => {
    ...
    return SELECTABLE_DOC_TYPES.has(fallback) ? fallback : "facture";
  };
  ...
  const createSourceTypeDialogController = () => {
    ...
    const renderChoices = (choices, fallbackDocType) => {
      ...
      const preferredDocType = normalizeDocType(
        fallbackDocType || choiceList[0]?.docType || "facture",
        "facture"
      );
          ...
          <div class="be-source-document-picker-modal__toolbar">
            <div class="be-source-document-picker-modal__filters">
              <label id="${SUPPLIER_FIELD_ID}" class="be-source-document-picker-modal__supplier" for="${SUPPLIER_INPUT_ID}">
                <span id="${SUPPLIER_LABEL_ID}" class="be-source-document-picker-modal__search-label">Client</span>
  ...
  const createModalController = () => {
    ...
    const loadSupplierOptions = async () => {
      ...
      if (w.electronAPI?.searchClients) {
        ...
        try {
          ...
          console.warn("bs source supplier list failed", err);
        }
      }
    ...
    const syncStatus = () => {
      if (!hasSelectedSupplier()) {
        setStatus("Selectionnez un client pour charger les documents.");
        return;
      }
      if (state.loading) {
      ...
      if (!total) {
        if (hasActiveFilters()) {
          setStatus(`Aucun document trouve pour ce client avec ${buildFilterSummary()}.`);
          return;
        }
        setStatus(`Aucun document disponible pour ce client (${sourceLabel()}).`);
        return;
      }
      const selected = selectedCount();
    ...
    const renderEntries = () => {
      ...
      if (!hasSelectedSupplier()) {
        gridEl.appendChild(createEmptyStateNode("Selectionnez un client pour afficher les documents."));
        syncActionButtons();
        syncStatus();
        syncPagerControls();
        return;
      ...
      if (!filteredEntries.length) {
        const emptyText = hasActiveFilters()
          ? `Aucun document trouve pour ce client avec ${buildFilterSummary()}.`
          : `Aucun document pour ce client (${sourceLabel()}).`;
    ...
    const resolveInitialSelectedKeys = (selection, activeDocType) => {
      ...
      const docType = normalizeDocType(
        raw.docType || activeDocType || "facture",
  ...
  const chooseDocumentSource = async ({
    ...
    try {
      ...
      console.warn("bs source document chooser failed", err);
      return fallback;
    }
  ...
  const openSourceDocumentPicker = async (trigger = null, options = {}) => {
    ...
    const fallbackDocType = normalizeDocType(
      options.fallbackDocType || selection?.docType || choices[0]?.docType || "facture",
  ...
  AppInit.BonSortieSourceDocumentPicker = {
