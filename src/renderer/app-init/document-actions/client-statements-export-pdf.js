(function (w) {
  ...
  const buildClientStatementsExportCss = () => {
    ...
    const localCss = `
      .client-statements-export-preview {
        display: block;
      }
      .client-statements-export-preview .pdf-page {
        width: 210mm;
        min-height: 296.5mm;
        height: auto;
        display: block;
      ...
      .client-statements-export-preview .report-tax-preview__heading,
      .client-statements-export-preview .client-statements-export-preview__meta {
        page-break-inside: avoid;
        break-inside: avoid;
      }
      .client-statements-export-preview__meta {
        margin-bottom: 10px;
      }
      .client-statements-export-preview__table-wrap {
        width: 100%;
        overflow: visible;
        page-break-inside: auto;
        break-inside: auto;
      }
      .client-statements-export-preview__table {
        width: 100%;
        border-collapse: collapse;
        font-size: 11px;
      }
      .client-statements-export-preview__table tbody {
        font-size: 11px;
      }
      .client-statements-export-preview__table thead {
        display: table-header-group;
      }
      .client-statements-export-preview__table th,
      .client-statements-export-preview__table td {
      ...
      .client-statements-export-preview__table tr {
        page-break-inside: avoid;
        break-inside: avoid;
      }
      .client-statements-export-preview__table thead th {
        background: #eff6ff;
        color: #0f172a;
        text-align: left;
      }
      .client-statements-export-preview__table .is-right {
        text-align: right;
      }
      .client-statements-export-preview__empty td {
        text-align: center;
        color: #64748b;
      }
    `;
  ...
  const buildRowsHtml = (rows) => {
    ...
    if (!list.length) {
      return `
        <tr class="client-statements-export-preview__empty">
          <td colspan="4">Aucun client pour cette periode.</td>
        </tr>
      `;
    }
      ...
      .map(
        (row) => `
          <tr>
            ...
            <td class="is-right">${safeHtml(row.totalDebitLabel || "-")}</td>
            <td class="is-right">${safeHtml(row.totalCreditLabel || "-")}</td>
            <td class="is-right">${safeHtml(row.soldLabel || "-")}</td>
          </tr>
        `
      )
      .join("");
  ...
  const buildClientStatementsExportHtml = (snapshot = {}) => {
    ...
    const totalDebitLabel = safeHtml(snapshot.totalDebitLabel || "0.000");
    const totalCreditLabel = safeHtml(snapshot.totalCreditLabel || "0.000");
    const totalSoldLabel = safeHtml(snapshot.totalSoldLabel || "0.000");
    const rowsHtml = buildRowsHtml(snapshot.rows);

    return `
      <div class="pdf-preview-root report-tax-preview__root client-statements-export-preview">
        <div class="pdf-page report-tax-preview__page">
          <div class="report-tax-preview__heading">
            <h1 class="pdf-title report-tax-preview__title">Solde clients</h1>
          </div>
          <div class="pdf-divider"></div>
          <div class="pdf-grid-2 report-tax-preview__meta client-statements-export-preview__meta">
            ...
            <div class="report-tax-preview__period">
              ...
              <p class="pdf-small pdf-meta-line">
                <span class="pdf-meta-label">Clients :</span>
                <span class="pdf-meta-value">${safeHtml(String(rowCount))}</span>
              </p>
              <p class="pdf-small pdf-meta-line">
                <span class="pdf-meta-label">Total debit :</span>
                <span class="pdf-meta-value">${totalDebitLabel}</span>
              </p>
              <p class="pdf-small pdf-meta-line">
                <span class="pdf-meta-label">Total credit :</span>
                <span class="pdf-meta-value">${totalCreditLabel}</span>
              </p>
              <p class="pdf-small pdf-meta-line">
                <span class="pdf-meta-label">Solde global :</span>
                <span class="pdf-meta-value">${totalSoldLabel}</span>
              </p>
            </div>
          </div>
          <div class="client-statements-export-preview__table-wrap">
            <table class="client-statements-export-preview__table">
              <thead>
                <tr>
                  <th>Client</th>
                  <th class="is-right">Total debit</th>
                  <th class="is-right">Total credit</th>
                  <th class="is-right">Solde client</th>
                </tr>
              </thead>
  ...
  const resolveClientStatementsPdfDir = async () => {
  ...
  const buildClientStatementsFilename = (startDate, endDate, index) => {
    ...
    return `RCS-${start}-${end}-${safeIndex}`;
  };

  const exportClientStatementsPdf = async ({ snapshot = {} } = {}) => {
    ...
    const saveDir = await resolveClientStatementsPdfDir();
    if (!saveDir) {
    ...
    const html = buildClientStatementsExportHtml(snapshot);
    const css = buildClientStatementsExportCss();
    let index = 1;
    let res = null;
    while (index <= 999) {
      const filename = buildClientStatementsFilename(snapshot.startDate, snapshot.endDate, index);
      try {
  ...
  AppInit.ClientStatementsExportPdf = {
    buildClientStatementsExportCss,
    buildClientStatementsExportHtml,
    exportClientStatementsPdf
  };
})(window);
