(function (w) {
  ...
  const renderContent = () => {
    ...
    const pdf = AppInit.ClientStatementsExportPdf;
    const snapshot = previewState.snapshot || {};
    if (!pdf || typeof pdf.buildClientStatementsExportHtml !== "function") {
    ...
    previewState.content.innerHTML = pdf.buildClientStatementsExportHtml(snapshot);
    updateActions();
  };
  ...
  const exportPreviewPdf = async () => {
    const pdf = AppInit.ClientStatementsExportPdf;
    if (!pdf || typeof pdf.exportClientStatementsPdf !== "function") {
      await showDialogMessage("Export PDF indisponible.", "Export PDF");
      return;
    }
    const res = await pdf.exportClientStatementsPdf({ snapshot: previewState.snapshot || {} });
    if (res?.ok) {
  ...
  const printPreview = async () => {
    const pdf = AppInit.ClientStatementsExportPdf;
    if (!pdf || !w.electronAPI?.printHTML) {
      await showDialogMessage("Impression indisponible.", "Impression");
      return;
    }
    if (
      typeof pdf.buildClientStatementsExportHtml !== "function" ||
      typeof pdf.buildClientStatementsExportCss !== "function"
    ) {
    ...
    try {
      const res = await w.electronAPI.printHTML({
        html: pdf.buildClientStatementsExportHtml(snapshot),
        css: pdf.buildClientStatementsExportCss(),
  ...
  const ensurePreviewModal = () => {
    ...
    overlay.id = "clientStatementsDetailModal";
    overlay.className = "swbDialog doc-history-modal report-tax-modal";
    overlay.hidden = true;
    overlay.setAttribute("aria-hidden", "true");
    overlay.innerHTML = `
      <div class="swbDialog__panel doc-history-modal__panel pdf-preview-modal__panel report-tax-modal__panel" role="dialog" aria-modal="true" aria-labelledby="clientStatementsDetailTitle">
        <div class="swbDialog__header">
          <div id="clientStatementsDetailTitle" class="swbDialog__title">Solde clients</div>
          <button id="clientStatementsDetailClose" type="button" class="swbDialog__close" aria-label="Fermer">
        ...
        <div class="pdf-preview-modal__body report-tax-modal__body swbDialog__msg">
          <div id="clientStatementsDetailContent" class="pdf-preview-modal__content report-tax-modal__content"></div>
        </div>
        <div class="pdf-preview-modal__actions report-tax-modal__actions">
          <div class="pdf-preview-modal__buttons report-tax-modal__buttons">
            <button id="clientStatementsDetailCloseFooter" type="button" class="client-search__edit">Fermer</button>
            <button id="clientStatementsDetailPrint" type="button" class="client-search__addSTK" disabled aria-disabled="true">Imprimer Rapport</button>
            <button id="clientStatementsDetailExport" type="button" class="client-search__edit" disabled aria-disabled="true">Exporter PDF</button>
            <button id="clientStatementsDetailOpenFolder" type="button" class="client-search__edit doc-history__open-folder" title="Ouvrir le dossier PDF" aria-label="Ouvrir le dossier PDF" disabled aria-disabled="true">
    ...
    previewState.content = overlay.querySelector("#clientStatementsDetailContent");
    previewState.title = overlay.querySelector("#clientStatementsDetailTitle");
    previewState.closeBtn = overlay.querySelector("#clientStatementsDetailClose");
    previewState.closeFooterBtn = overlay.querySelector("#clientStatementsDetailCloseFooter");
    previewState.exportBtn = overlay.querySelector("#clientStatementsDetailExport");
    previewState.printBtn = overlay.querySelector("#clientStatementsDetailPrint");
    previewState.openFolderBtn = overlay.querySelector("#clientStatementsDetailOpenFolder");
  ...
  const showClientStatementsExportPreview = async ({ snapshot = {} } = {}) => {
    ensurePreviewModal();
    previewState.snapshot = snapshot && typeof snapshot === "object" ? { ...snapshot } : {};
    previewState.lastExportPath = "";
    if (previewState.title) previewState.title.textContent = "Solde clients";
  ...
  AppInit.ClientStatementsExportPreview = {
    ensurePreviewModal,
    showClientStatementsExportPreview,
    closePreviewModal
  };
})(window);
